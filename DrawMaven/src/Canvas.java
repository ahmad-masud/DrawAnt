...
public final class Canvas extends JFrame {
    ...
    private final JScrollBar posXSlider;
    private final JScrollBar posYSlider;
    ...
    public Canvas() {
        ...
        posXSlider = new JScrollBar(JScrollBar.HORIZONTAL, xOffset, 0, -100, 100);
        posXSlider.setEnabled(false);
        posXSlider.addAdjustmentListener(e -> {
            JScrollBar source = (JScrollBar) e.getSource();
            xOffset = (int) ((-source.getValue()*img.getWidth()*zoom)/100);
            repaint();
        });
        add(posXSlider, BorderLayout.SOUTH);
        
        posYSlider = new JScrollBar(JScrollBar.VERTICAL, yOffset, 0, -100, 100);
        posYSlider.setEnabled(false);
        posYSlider.addAdjustmentListener(e -> {
            JScrollBar source = (JScrollBar) e.getSource();
            yOffset = (int) ((-source.getValue()*img.getHeight()*zoom)/100);
            repaint();
        });
        add(posYSlider, BorderLayout.EAST);
    ...
    public void setZoom(double zoom) {
        this.zoom = zoom;
        if (zoom*img.getWidth() > getContentPane().getWidth()) {
            posXSlider.setEnabled(true);
        } else {
            posXSlider.setEnabled(false);
        }
        
        if (zoom*img.getHeight() > getContentPane().getHeight()) {
